...
export default function Home() {
  ...
  const { mutate, post } = useApi<Guestbook[]>("/guestbook");
  ...
  return (
    <div>
      ...
      <main className={styles.main}>
        {session?.user && (
          <h2 className='my-4 text-2xl sm:text-3xl font-bold py-3'>{`Welcome Back , ${session.user.name} !`}</h2>
        )}
          ...
          {!session && (
            <div className='-mt-20 px-5'>
              <div className='mb-6 flex flex-col justify-start flex-shrink-0 transform-none w-full  '>
                <p className=' tracking-wider text-6xl sm:text-8xl font-bold'>
                  Let's write a message for the visitors.
                </p>
              </div>
              <a
          ...
          {session?.user && (
            <div className='py-2'>
              <form className='sm:flex sm:gap-4 font-sans' onSubmit={submit}>
                ...
                <button
                  type='submit'
                  className='mt-4 flex w-full items-center justify-center rounded-xl bg-[#2C40F7] px-5 py-3 text-white transition focus:outline-none focus:ring sm:mt-0 sm:w-auto'
                >
